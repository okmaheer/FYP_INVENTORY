        ...
        <table style="width: 100%; border: 1px solid var(--summary-row-border);">
            <tbody>
                <tr style="border: 1px solid var(--summary-row-border);background-color: var(--summary-row-bg);">
                    <td style="width: 88%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);">Addon Event Total</td>
                    <td style="width: 12%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);">{{AccountHelper::number_format( ($addonModel->no_person * $addonModel->rate_per_head) + $addon_foodTotal + $addon_addonTotal )}}</td>
                </tr>
                @if (!empty($addonModel->booking_detail))
                <tr style="border: 1px solid var(--summary-row-border);background-color: var(--summary-row-bg);">
                    ...
                    <td style="width: 12%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);"><small>(+)</small>{{AccountHelper::number_format($addonModel->additional_charges)}}</td>
                </tr>
                @endif
                <tr style="border: 1px solid var(--summary-row-border);background-color: var(--summary-row-bg);">
                    <td style="width: 88%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);">Addon Event Grand Total</td>
                    <td style="width: 12%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);">{{AccountHelper::number_format(MarqueeHelper::bookingGrandAmount($addonModel->id) + $tax_amount)}}</td>
                </tr>
                @if(!empty($addonModel->tax_id))
                    @php $tax_amount = $addonModel->total_tax @endphp
                    <tr style="border: 1px solid var(--summary-row-border);background-color: var(--summary-row-bg);">
                        <td style="width: 88%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);">Tax ({{ $addonModel->tax->tax_name }})</td>
                        <td style="width: 12%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);"><small>(+)</small>{{AccountHelper::number_format($addonModel->total_tax)}}</td>
                    </tr>
                @endif
                @if(!empty($addonModel->misc_discount_type))
                <tr style="border: 1px solid var(--summary-row-border);background-color: var(--summary-row-bg);">
                    <td style="width: 88%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);">
                        Discount ({{ MarqueeHelper::eventDiscount($addonModel->misc_discount_type) }})&nbsp;
                        @if ($addonModel->misc_discount_type === 2)
                            {{ $addonModel->misc_discount_total }}%
                        @elseif ($addonModel->misc_discount_type === 3)
                            {{ round($addonModel->misc_discount_total,0) }} Persons
                        @endif
                    </th>
                    <td style="width: 12%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);"><small>(-)</small>{{AccountHelper::number_format($addonModel->misc_discount_value)}}</td>
                </tr>
                @endif
                @if(!$addonModel->stageDecorations->isEmpty())
                    <tr style="border: 1px solid var(--summary-row-border);background-color: var(--summary-row-bg);">
                ...
                <tr style="border: 1px solid var(--summary-row-border);background-color: var(--summary-row-bg);">
                    ...
                    <td style="width: 12%; font-weight:bold; text-align: right; padding: .3rem;border: 1px solid var(--summary-row-border);">{{AccountHelper::number_format( MarqueeHelper::bookingTotalNetAmount($addonModel->id) ) }}</td>
                </tr>
            </tbody>
        </table>
    </div>
