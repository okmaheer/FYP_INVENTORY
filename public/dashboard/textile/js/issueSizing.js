...
function calculate_store(sl) {
    ...
    var gr_tot = 0;
    var dis = 0;
    var ttl_count = $("#ttl_count_" + sl).val();
    // var vendor_rate = $("#product_rate_" + sl).val();

    var total_price = ttl_count * 2100;
...
function product_pur_or_list(sl) {
    ...
    var options = {
        ...
        select: function(event, ui) {
            ...
            var available_amount = 'available_amount_' + sr_no;

            var form2Data = {
                product_id: ui.item.value,
                supplier_id: supplier_id
            };
            var ajax2url = '/dashboard/textile/retrieve_purchase_data';
            ...
            $.ajax({
                ...
                success: function(data) {
                    console.log(data);
                    $('#' + available_quantity).val(data.purchaseBags);
                    $('#' + available_weight).val(data.total_weight);
                    $('#' + available_amount).val(data.totalAmount);
                },
                // error: function (data) {
                //     console.log(data);
                // }
